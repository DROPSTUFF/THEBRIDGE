...
void ofxIpCamera::update() {
	...
	//assert(bGrabberInited); //build-error? Geen idee waarom.
...
bool ofxIpCamera::grabOneFrame(Poco::Net::HTTPClientSession& client) {
    ...
    if ( bVerbose ){
        cout << "ofxIpCamera:grabOneFrame: image format " << ofToString(format) << endl;
    }
    if(format != FIF_JPEG) {
        //geen jpeg check...
